...
# 將計時器變數設定為當下時間之後 200 毫秒
seTimer = utime.ticks_add(utime.ticks_ms(), 200)
while True:
    # 持續檢查是否收到新指令
    ESP8266WebServer.handleClient()
    # 當計時器變數與現在的時間差小於 0 則執行任務
    if utime.ticks_diff(seTimer, utime.ticks_ms()) < 0:
        try:
            # 使用 i2c 來向特定頻道 slave 要求長度 32 bytes 資料
            readStr = i2c.readfrom(ard_i2c_addr, 32)
            # 略過無用的空資料
            logStr += readStr[:readStr.find(b'\xff')]
        except:
            pass
